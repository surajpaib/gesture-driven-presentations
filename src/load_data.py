...
def create_video_data_label(interpolation_frames, noise_parameters):
    ...
    for label, folder in enumerate(os.listdir(xml_folder)):
        ...
        for file in os.listdir(xml_folder + '/' + folder):
            ...
            matrix = video_data.generate_matrices()
            for frame in matrix:
                data.append(frame)
                labels.append(label)
...
def load_video_data_labels(interpolation_frames, noise_parameters):
    path = 'interpolation_' + str(interpolation_frames) + '_noise_' + str(
        noise_parameters) + '.pkl'
    try:
        ...
        video_data, video_labels = create_video_data_label(interpolation_frames, noise_parameters)
